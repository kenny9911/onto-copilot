...
class Segment:
    ...
    def rows(self, index: EvidenceIndex) -> tuple[list[dict[str, Any]], list[str]]:
        ...
        for cid in self.chunk_ids:
            c = index.get(cid)
            if c is not None and isinstance(c.raw, dict):
                rows.append(c.raw)
                cites.append(c.cite())
        return rows, cites
...
def segment_corpus(index: EvidenceIndex, docs: list[Any]) -> list[Segment]:
    ...
    for i, ((fname, key), ids) in enumerate(sorted(merged.items())):
        ...
        for part in range(0, max(1, len(ids)), SEGMENT_CHUNKS):
            ...
            if not slice_ids:
                continue
            suffix = f"{i}_{part // SEGMENT_CHUNKS}"
            seg = Segment(
                key=f"s{suffix}", file_name=fname,
                label=key + (f"（第 {part // SEGMENT_CHUNKS + 1} 部分）"
                             if len(ids) > SEGMENT_CHUNKS else ""),
                chunk_ids=slice_ids)
            # 形状只看列的取值分布，不看内容语义 —— 冻结计划的边界没有被破坏。
            rows, _ = seg.rows(index)
            seg.shape = infer_shape(rows)
            out.append(seg)
    return out
...
class ExtractSegment(NodeHandler):
    ...
    def task(self, inputs: dict[str, Any]) -> str:
        ...
        want: list[str] = []
        if Yield.PROPERTIES in shape.yields:
            want.append("**每一行字段抽成一个 PropertyType**，口径（税/时间粒度/"
                        "口径主体/币种）写进 definition")
        ...
        if Yield.ACTIONS in shape.yields and not n_act:
            want.append("对象上的**行动**（谁在什么条件下能改这条数据）")
        if not want:
...
def build_oir(data: dict[str, Any], index: EvidenceIndex | None = None) -> OIR:
    ...
    for a in data.get("actions", ()):
        ...
        if rid in oir.actions:
            continue
        host = (by_api.get(str(a.get("object") or "").lower())
                or by_display.get(str(a.get("object_display") or "").strip()))
        ev = _prov(a, index)
...
def _looks_like_prose(name: str) -> bool:
    if len(name) > 24:
        return True
    return any(h in name for h in _PROSE_HINTS)
...
class CoverageCritic(Critic):
    ...
    def _check(self, draft: Any, segment: Segment) -> list[Finding]:
        ...
        expected = {
            Yield.PROPERTIES: (props, "属性"),
            Yield.OBJECTS: (objs, "对象"),
            Yield.ACTIONS: (acts, "行动"),
        }
        for y, (got, cn) in expected.items():
            if shape.expects(y) and not got:
                out.append(Finding(
