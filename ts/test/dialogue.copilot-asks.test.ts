...
describe("按钮预填的原话必须能过意图闸", () => {
  ...
  // ── 现场那一句（2026-09-04 用户截图） ──────────────────────────
  //
  // 他说「把"…-v2.xlsx"放进通用的知识库里」，闸不认，模型只能回一句
  // 「当前对话环境未被授予对项目知识库的写入和归档权限」—— 一句既不准确
  // （权限没问题）又没给出路的话。原因是「知识库」这个词的写法卡得太死：
  // 动词后面必须紧跟着它，中间不能有修饰语（通用的）、后面不能有方位词（里）。
  it("「放进通用的知识库里」要认 —— 修饰语和方位词都不该卡住一句正常中文", async () => {
    expect(await authorizes(
      '把"采购制度.md"放进通用的知识库里', "document.promote",
      { session_file_name: "采购制度.md" },
    )).toBe(true);
  });

  it("动词的自然变体都要认：存进 / 存到 / 放入 / 收进", async () => {
    for (const verb of ["存进", "存到", "放入", "收进"]) {
      expect(await authorizes(
        `把采购制度.md${verb}知识库里`, "document.promote",
        { session_file_name: "采购制度.md" },
      )).toBe(true);
    }
  });
