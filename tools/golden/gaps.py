...
ENUM_CASES: list[tuple[str, list[Chunk]]] = [
    ...
    # `col1` 是解析器给无表头的列**编出来**的名字，不是材料里的取值域。判据必须
    # 看**剥编号之前**的原名：剥完变成 `col`，`col\d` 就再也够不着 —— 那个洞会把
    # 一个凭空拼出来的列名当成真取值域问给业务方。
    ("不命中：拼接出来的列名 col1 —— 判据看的是剥编号之前的原名",
     [sheet("col1：甲类、乙类、丙类；")]),
    # 反过来：`3.` 是列表编号，剥掉之后「进度状态」是个正经取值域。把整条
    # `_BAD_NAME` 拿去卡未剥的名字，会连这条一起毙掉 —— 那是反方向的漏问。
    ("命中：名字前挂着列表编号，编号剥掉之后照样问",
     [sheet("3.进度状态：未开始、执行中、已完成；")]),
    ("不命中：中文数字编号 —— strip 剥不掉，靠中文数字那条判据",
     [sheet("一.进度状态：未开始、执行中、已完成；")]),
